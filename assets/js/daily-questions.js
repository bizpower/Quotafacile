...
(function () {
  ...
  /* "Il costo di la diaria da ricovero": l'errore c'era in due
     risposte su quattro, cioe' in centoquarantasette pagine, e lo
     legge chiunque apra una domanda del giorno. I temi sono
     scritti con l'articolo davanti perche' serve alla domanda
     ("Quanto costa la diaria?"), quindi nella risposta va fatta
     la preposizione articolata invece di incollare "di". */
  const DI = { "l'": "dell'", "la ": "della ", "le ": "delle ",
               "il ": "del ", "lo ": "dello ", "gli ": "degli ", "i ": "dei " };
  const di = t => {
    for (const a in DI) if (t.startsWith(a)) return DI[a] + t.slice(a.length);
    return "di " + t;
  };
